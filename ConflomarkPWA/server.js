...
app.get('/', (req, res) => {
    res.send([{ messages: 'HOLA A TODOS' }]);
});

app.listen(2100, 'localhost', function() {
    console.log("CONEXIÓN EXITOSA CON EL PUERTO: 2100");
});
...
/////Consulter users////// done
app.get('/ubigeo', function(req, res) {
    "use strict";
    ...
    oracledb.getConnection(connAttrs, function(err, connection) {
        ...
        connection.execute("SELECT * FROM UBIGEO", {}, {
            outFormat: oracledb.OBJECT // Return the result as Object
        }, function(err, result) {
            if (err) {
                res.set('Content-Type', 'application/json');
